...
def save_model(langchain_asset, path, mlflow_model=None, metadata=None, action_records=None, session_analysis=None):
    # Treat this more as "ensure import" so we don't lose easy typing?
    _ = import_pandas()

    # Replace all this with real mlflow.langchain.log_model(...)
    # Decide pyfunc representation for repeated invocation (.run())
    langchain_asset_path = Path(path, "model.json")
    try:
    ...
    mlflow_model.save(Path(path,  MLMODEL_FILE_NAME))
